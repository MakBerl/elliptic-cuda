...
void cpuJacobi (float * input, float * output, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N, int jumlahElemen) {
...
void cpuPGS(float * input, float * output, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N, int jumlahElemen) {
...
void cpuPSOR(float * input, float * output, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N, int jumlahElemen, float omega) {
	//float A = 0, B = 0, C = 0, D = 0;
	for (int i = 0; i < jumlahElemen; i++) {
		//tetap pada batas:
		//y = 0 --> i % N = 0
...
__global__ void gpuJacobi (float * input, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N) {
	//thread ID
...
__global__ void gpuPGSRed (float * input, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N) {
	float hasil = 0;
	//thread ID
	int tidx = (blockIdx.x * blockDim.x) + threadIdx.x;
	int tidy = (blockIdx.y * blockDim.y) + threadIdx.y;
...
__global__ void gpuPGSBlack (float * input, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N) {
	float hasil = 0;
	//thread ID
	int tidx = (blockIdx.x * blockDim.x) + threadIdx.x;
	int tidy = (blockIdx.y * blockDim.y) + threadIdx.y;
...
__global__ void gpuPSORRed (float * input, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N, float omega) {
	float hasil = 0;
	//thread ID
	int tidx = (blockIdx.x * blockDim.x) + threadIdx.x;
	int tidy = (blockIdx.y * blockDim.y) + threadIdx.y;
...
__global__ void gpuPSORBlack (float * input, float * error, char * poisson, char ad_n, char ad_e, char ad_s, char ad_w, int N, float omega) {
	float hasil = 0;
	//thread ID
	int tidx = (blockIdx.x * blockDim.x) + threadIdx.x;
	int tidy = (blockIdx.y * blockDim.y) + threadIdx.y;
...
int main (int argc, char** argv) {
	...
	char ad_n = 0, ad_e = 0, ad_s = 0, ad_w = 0;
	char *awal_poi;
	char *host_poi;
	char *dev_poi;
	float omega = 1.1;
	...
	//inialisasi
	awal_poi = (char *)malloc(sizeof(char) * totalElemen);
	...
	for(int i = 0; i<totalElemen; i++) {
		//kondisi batas
		if(i==((SISI_MATRIKS*SISI_MATRIKS/2) + SISI_MATRIKS/2)) {
			awal_in[i] = 100;
		}
		else {
			awal_in[i] = 0;
	...
	host_poi = (char *)malloc(sizeof(float) * totalElemen);
	...
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		cpuJacobi(host_in, host_out, host_err, host_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS, totalElemen);
	...
	total_error = -1;
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		cpuPGS(host_in, host_out, host_err, host_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS, totalElemen);
	...
	total_error = -1;
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		cpuPSOR(host_in, host_out, host_err, host_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS, totalElemen, omega);
	...
	printf("GPU Jacobi....");
	//alokasi memori domain
	cudaMalloc( (void **)&dev_poi, sizeof(char) * totalElemen);
	cudaMalloc( (void **)&dev_in, sizeof(float) * totalElemen) ;
	cudaMalloc( (void **)&dev_err, sizeof(float) * totalElemen);

	//copy data dari matriks awal ke domain
	cudaMemcpy(dev_poi, awal_poi, totalElemen*sizeof(char), cudaMemcpyHostToDevice);
	...
	total_error = -1;
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		gpuJacobi<<<jumlahBlock, threadPerBlock>>>(dev_in, dev_err, dev_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS);
	...
	total_error = -1;
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		gpuPGSRed<<<jumlahBlock, threadPerBlock>>>(dev_in, dev_err, dev_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS);
		gpuPGSBlack<<<jumlahBlock, threadPerBlock>>>(dev_in, dev_err, dev_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS);
	...
	total_error = -1;
	while ((total_error == -1) || (total_error > MAX_ERROR)) {
		gpuPSORRed<<<jumlahBlock, threadPerBlock>>>(dev_in, dev_err, dev_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS, omega);
		gpuPSORBlack<<<jumlahBlock, threadPerBlock>>>(dev_in, dev_err, dev_poi, ad_n, ad_e, ad_s, ad_w, SISI_MATRIKS, omega);
